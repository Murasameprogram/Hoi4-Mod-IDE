...
### 2026-02-25
**本次更新内容**：
- **地块解析插件**：目前实现功能为根据地块颜色解析出唯一的地块ID
- **高位图预览插件**：将钢铁雄心4的高位图还原为3D白模，提供在游戏外预览地形的能力
- **局域网多人协同插件**：自由选择是否安装插件以启动软件的多人协同编辑能力，该插件提供，在局域网内自动进行组网（需同时运行软件和安装此插件），采用增量传输机制，大幅降低网络带宽压力，目前暂未实装加密传输，数据在局域网内均采用明文传输机制，建议谨慎使用！
